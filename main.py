...
def exact_inference_engine_test(bayesian_network: BayesianNetwork, queries: List[str], evidence: List[Tuple[str, str]],
    ...
    string: str = engine.elim_ask(queries, evidence).output_to_latex_with_query(queries, title)
    # multiprocessing.Lock().acquire()
    print(string)
    print(engine.iterations)
...
# Gibbs sampling: TODO
def main() -> None:
    networks: List[str] = ["insurance.bif"]  # ["alarm.bif", "child.bif", "hailfinder.bif", "insurance.bif", "win95pts.bif"]
    ...
    for network in networks:
        ...
        for i, evidence in enumerate(evidences[network]):
            ...
            print()
            for j, query in enumerate(queries[network]):
                # exact_inference_engine_test(bayesian_network, [query], evidence,
                #                             "Probability distribution of " + query + " in network " + network + " with evidence set " + str(
                #                                 i))
                processes.append(multiprocessing.Process(target=exact_inference_engine_test, args=(bayesian_network, [query], evidence, "Probability distribution of " + query + " in network " + network + " with evidence set " + str(i),),
                                                         name=f"insurance_process_{j}"))
